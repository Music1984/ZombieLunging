...
namespace ZombieLunging
{
	public class CustomZombie : MonoBehaviour
	{
		...
		public void OnPlayerHurt(HurtingEventArgs ev)
		{
			...
			if (ev.Target.Id == playerReferenceHub.playerId && ev.Target.Role == RoleType.Scp0492 && ev.Handler.Type == DamageType.Scp207) ev.Amount = 0.0f;
		...
		private IEnumerator<float> ForceSlowDown(float totalWaitTime, float interval)
		{
			...
			{
				if (!sinkHole.IsEnabled) playerReferenceHub.playerEffectsController.EnableEffect<SinkHole>();
				waitedTime += interval;
		...
		private IEnumerator<float> ForceSpeedUp(float totalWaitTime, float interval)
		{
			...
			while ((double)waitedTime < (double)totalWaitTime)
			{
				if (!scp207.IsEnabled) playerReferenceHub.playerEffectsController.ChangeByString("scp207", (byte)Plugin.instance.Config.ColaIntensity);
				waitedTime += interval;
				yield return Timing.WaitForSeconds(interval);
			}
			playerReferenceHub.playerEffectsController.DisableEffect<Scp207>();
